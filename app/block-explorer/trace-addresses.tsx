...
export default function TraceAddresses() {
  const router = useRouter();
  return (
    <LinearGradient colors={["#1d0b3b", "#7a2bca"]} style={styles.container}>
      <BackHeader title="Trace Addresses" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          <View style={styles.iconContainer}>
            <Ionicons name="map-outline" size={40} color="#6366f1" />
          </View>
          <Text style={styles.title}>Following the Money</Text>
          <Text style={styles.subtitle}>Track balances across addresses</Text>
          <View style={styles.divider} />
          <Text style={styles.paragraph}>
            Addresses receive and send funds creating a verifiable trail. Explorers aggregate this data to visualize balances and histories.
          </Text>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  iconContainer: { width: 70, height: 70, borderRadius: 20, backgroundColor: 'rgba(99, 102, 241, 0.25)', justifyContent: 'center', alignItems: 'center', marginBottom: 20, alignSelf: 'center' },
