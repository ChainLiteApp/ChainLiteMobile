...
export default function UnderstandingBlocks() {
  const router = useRouter();
  return (
    <LinearGradient colors={["#1d0b3b", "#7a2bca"]} style={styles.container}>
      <BackHeader title="Understand Blocks" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          <View style={styles.iconContainer}>
            <Ionicons name="cube-outline" size={40} color="#a78bfa" />
          </View>
          <Text style={styles.title}>What is in a Block?</Text>
          <Text style={styles.subtitle}>Headers, nonce, previous hash, and more</Text>
          <View style={styles.divider} />
          <Text style={styles.paragraph}>
            Blocks contain a header with metadata (timestamp, previous hash, nonce, index) and a list of validated transactions. The hash of the block secures its contents.
          </Text>
        </View>
      </ScrollView>
      <View style={styles.footer}>
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={() => router.push('/block-explorer/view-transactions')}>
            <Text style={styles.primaryButtonText}>Next: View Transactions</Text>
...
const styles = StyleSheet.create({
  ...
  iconContainer: { width: 70, height: 70, borderRadius: 20, backgroundColor: 'rgba(167, 139, 250, 0.2)', justifyContent: 'center', alignItems: 'center', marginBottom: 20, alignSelf: 'center' },
  ...
  secondaryButtonText: { color: '#a78bfa', fontSize: 16, fontWeight: '600' },
});
