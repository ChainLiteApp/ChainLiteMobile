...
export default function ConnectNodes() {
  ...
  return (
    <LinearGradient colors={["#0b2040", "#0a1730"]} style={styles.container}>
      <BackHeader title="Connect Nodes" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          <View style={styles.iconContainer}>
            <Ionicons name="share-social-outline" size={40} color="#60a5fa" />
          </View>
          <Text style={styles.title}>Peer Discovery</Text>
          <Text style={styles.subtitle}>How nodes find each other</Text>

          <View style={styles.divider} />
          <Text style={styles.paragraph}>
            Nodes connect over a peer-to-peer network. Each node keeps a list of known peers and attempts to establish connections to exchange data.
          </Text>
        </View>
      </ScrollView>
      <View style={styles.footer}>
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={styles.primaryButton} onPress={() => router.push('/network-builder/sync-chain')}>
            <Text style={styles.primaryButtonText}>Next: Sync the Chain</Text>
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: '#60a5fa', borderRadius: 14, padding: 16, flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 8 },
  ...
  secondaryButtonText: { color: '#60a5fa', fontSize: 16, fontWeight: '600' },
});
